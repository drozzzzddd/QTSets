...
Integer* int_set = new Integer;
Integer* int_set_2 = new Integer;
StringSet* str_set = new StringSet;
StringSet* str_set_2 = new StringSet;
...
{
    if(ui->comboBox->currentText() == "Integers") {
       Set* result = int_set->add(int_set_2);
       Integer* output = dynamic_cast<Integer*>(result);
       QString buffer;
       for (auto el : output->Get()){
           buffer += QString::number(el) + '\n';
       }
       ui->result_text->setPlainText(buffer);
    } else {
        Set* result = str_set->add(str_set_2);
        StringSet* output = dynamic_cast<StringSet*>(result);
        QString buffer;
        std::vector<QString> vec = output->Get();
        for (size_t i = 0; i < vec.size(); ++i){
            buffer += vec[i] + '\n';
        }
        ui->result_text->setPlainText(buffer);
...
{
    if(ui->comboBox->currentText() == "Integers") {
        Set* result = int_set->mult(int_set_2);
        Integer* output = dynamic_cast<Integer*>(result);
       QString buffer;
       for (auto el : output->Get()){
           buffer += QString::number(el) + '\n';
       ...
       ui->result_text->setPlainText(buffer);
        ...
        Set* result = str_set->mult(str_set_2);
        StringSet* output = dynamic_cast<StringSet*>(result);
        QString buffer;
        std::vector<QString> vec = output->Get();
        for (size_t i = 0; i < vec.size(); ++i){
...
{
    if(ui->comboBox->currentText() == "Integers") {
        Set* result = int_set->sim_sub(int_set_2);
        Integer* output = dynamic_cast<Integer*>(result);
       QString buffer;
       for (auto el : output->Get()){
           buffer += QString::number(el) + '\n';
       ...
       ui->result_text->setPlainText(buffer);
        ...
        Set* result = str_set->sim_sub(str_set_2);
        StringSet* output = dynamic_cast<StringSet*>(result);
        QString buffer;
        std::vector<QString> vec = output->Get();
        for (size_t i = 0; i < vec.size(); ++i){
